...
parser = ArgumentParser(prog = 'inner-prokaryote-interactomes.R')
grid = read.delim("sh/grids/prokaryote-interactomes.txt")
...
# read complexes from EcoCyc
species_map = read.csv("data/GO/species-map.csv") %>% 
  # remove one unused proteome
  filter(!proteome %in% 'UP000001014')
proteome = filter(species_map, species == args$species) %>% pull(proteome)
gene_col = ifelse(proteome == 'UP000000625', 'gene_name', 'target')
complexes = paste0("data/prok_interactomes/EcoCyc/complexes_", 
                   proteome, ".csv") %>% 
  read.csv() %>%
  as_annotation_list(gene_col, 'complex')
