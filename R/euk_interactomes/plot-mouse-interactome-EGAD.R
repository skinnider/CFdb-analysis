...
# read EGAD results
egad_v1 = readRDS("data/euk_interactomes/EGAD/network-species=Mus_musculus-n_replicates=21-version=V1-n_features=1.rds")
egad_v2 = readRDS("data/euk_interactomes/EGAD/network-species=Mus_musculus-n_replicates=40-version=V2-n_features=1.rds")
sizes_v1 = attr(egad_v1, 'gene_set_sizes')
sizes_v2 = attr(egad_v2, 'gene_set_sizes')

# read tissue interactomes
tissue_names = c('Brain', 'Heart', 'Liver', 'Lung', 'Kidney', 'Muscle', 'Thymus')
tissue_files = paste0("data/euk_interactomes/EGAD/Skinnider2021-", 
                      tissue_names, ".rds")
tissues = map(tissue_files, readRDS) %>% setNames(tissue_names)

# define plotting grid
grid = tidyr::crossing(terms = 'GO', plot = c('v1_v2', 'tissues'))
for (grid_idx in seq_len(nrow(grid))) {
  ...
  if (plot == 'v1_v2') {
    ...
    df %<>%
      ...
      ungroup()
  } else if (plot == 'tissues') {
    v2 = egad_v2 %>% filter(between(n_proteins, 10, 100)) %>% 
      mutate(network = 'CF-MS')
    df = map(tissues, ~ filter(.x, between(n_proteins, 10, 100)) %>% 
  ...
  # p
  # save
  output_file = paste0("fig/euk_interactome/EGAD/mouse-plot=", plot, 
                       "-terms=", terms, ".pdf")
