...
tissue_files = list.files("data/resources/interactomes", full.names = TRUE, 
  ...
  extract(grepl('csv', .))
tissues = map(tissue_files, read.csv) %>%
  setNames(gsub("^.*-|\\.csv$", "", basename(tissue_files)))

# set up coexpression files
coexpr_names = c('Giansanti2022', 'Geiger2013')
coexpr_files = paste0("data/resources/proteomes/", coexpr_names, ".rds")

# read all matrices
mats = map(coexpr_files, ~ readRDS(.) %>% as.matrix()) %>%
  setNames(coexpr_names)

# calculate coexpression
cors = map(mats, ~ cor(t(.), use = 'p', method = 'pearson'))

# define plotting grid
grid = tidyr::crossing(matrix = names(cors), plot = c('v1_v2', 'tissues'))
for (grid_idx in seq_len(nrow(grid))) {
  ...
  p
  ggsave(paste0("fig/euk_interactomes/coexpression/mouse-", matrix, 
                "-", plot, "-spectrum.pdf"),
