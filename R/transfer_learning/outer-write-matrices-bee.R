...
# parse arguments
parser = ArgumentParser(prog = 'outer-write-matrices-bee.R')
...
chrom_file = "data/bee/chromatograms.rds"
chrom = readRDS(chrom_file)
replicates = names(chrom)
grid %<>% 
  tidyr::crossing(replicate = replicates) %>% 
  dplyr::select(replicate, everything())

# clean up grid
grid %<>%
  dplyr::select(replicate, analysis, metric, scale, transform, min_pairs, 
                normalize, missing)
...
output_dir = file.path(base_dir, "bee", "matrices")

# now, check for which parameters are already complete
grid0 = grid %>%
  mutate(output_dir = file.path(base_dir, "matrices", replicate),
         output_filename = paste0('matrix', 
                                  '-metric=', metric,
                                  '-scale=', scale,
                                  '-transform=', transform,
                                  '-normalize=', normalize,
  ...
  # set up input file
  mutate(input_file = chrom_file)

# write
grid_file = "sh/grids/write_matrices_bee.txt"
...
sh_file = '~/git/CFdb-analysis/sh/write-matrices-bee.sh'
write_sh(job_name = 'write-matrices',
         sh_file = sh_file,
         grid_file = grid_file,
         inner_file = 'R/transfer_learning/inner-write-matrices-bee.R',
         system = system,
         time = 24,
         cpus = 1,
         mem = 64)
